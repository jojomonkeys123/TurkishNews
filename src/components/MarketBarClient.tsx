...
import type { PiyasaSatiri } from "@/lib/market";

const YENILEME_MS = 45000;

export default function MarketBarClient({ initial }: { initial: PiyasaSatiri[] }) {
  const [markets, setMarkets] = useState(initial);
  const [guncelleme, setGuncelleme] = useState<Date | null>(null);

  useEffect(() => {
    setGuncelleme(new Date());
    let iptal = false;
    ...
    async function yenile() {
      try {
        ...
        if (!iptal && Array.isArray(data.ozet)) {
          setMarkets(data.ozet);
          setGuncelleme(new Date());
        }
      } catch {
        // sessizce geç, mevcut veriyi göstermeye devam et
      }
    }
  ...
  const guncellemeStr = guncelleme
    ? guncelleme.toLocaleString("tr-TR", {
        day: "2-digit",
        month: "short",
        year: "numeric",
        hour: "2-digit",
